...
function showView(name) {
  ...
  if (name === 'icon') viewIcon.classList.remove('hidden');
  else if (name === 'menu') viewMenu.classList.remove('hidden');
  else if (name === 'bar') viewBar.classList.remove('hidden');
}
...
async function startAzureSpeech(key, region) {
...
async function startMic() {
  ...
  if (azureConfig?.key && azureConfig?.region) {
    const ok = await startAzureSpeech(azureConfig.key, azureConfig.region);
    if (ok) return;
    useWhisperFallback = true;
    startWhisperFallback();
    return;
...
function renderHistory() {
  ...
  // Auto-scroll history to bottom if enabled
